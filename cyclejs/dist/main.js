...
},{"rx":5}],2:[function(require,module,exports){
...
require('./dist/rx.dom');
module.exports = Rx;
},{"./dist/rx.dom":3,"rx":5}],5:[function(require,module,exports){
...
function makeGridDriver(canvasElt) {
	...
	var canvas = typeof canvasElt === "string" ? document.querySelector(canvasElt) : canvasElt;
...
function makeMouseTracker(draggable) {
	...
	return function mouseTrack() {
		...
		var dragger$ = mouseDown$.flatMap(function (md) {
			...
			var mouseMove$ = Rx.DOM.mousemove(document).map(function (mm) {
				return { startX: md.clientX, startY: md.clientY, x: mm.clientX - md.clientX, y: mm.clientY - md.clientY };
			}).filter(function (p) {
				return p.x != p.y;
			});
			...
			var offsetFunctionMap = {
				horz: function horz(p) {
					return p.x;
				},
				vert: function vert(p) {
					return p.y;
				}
			};
			...
			var makeOutput = function makeOutput(p, hv) {
